...
bert_layer2 = TFBertModel.from_pretrained('klue/bert-base', from_pt=True)
bert_tokenizer2 = AutoTokenizer.from_pretrained('klue/bert-base')
...
output = bert_layer2([input_ids,attention_masks])
...
def coverup_swears(encoded, predict):
    ...
    for i in range(1,len(encoded)):
        ...
        elif encoded[i] == 1 or predict[i] == 'B' or predict[i] == 'I':
            res.append(32707)
        else:
...
def read_root(item: Item):
    #print('hi')
    print(item)
    s_inputs , s_attentions = bert_encode([bert_tokenizer2.tokenize(item.sentence)],352)
    isswear = modeltf2.predict([s_inputs,s_attentions])
    print(isswear)
    if isswear <0.3:
        return {"res":item.sentence}
    else:
